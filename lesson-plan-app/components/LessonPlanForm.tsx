...
import { BookOpen, Clock, Target, BookmarkIcon, Sparkles, GraduationCap, Loader2, X } from "lucide-react";
...
const LessonPlanForm = ({ isSubscribed }: { isSubscribed: boolean }) => {
  ...
  const isStepValid = (currentStep: number) => {
    switch (currentStep) {
      ...
      case 2:
        return isSubscribed
          ? (customTopic !== "" && customSubtopic !== "") || (formData.topic !== "" && formData.subtopic !== "")
          : formData.subtopic !== "";
      case 3:
  ...
  const renderStep = () => {
    switch (step) {
      case 1:
        return (
          <motion.div key={"step1"} initial="hidden" animate="visible" exit="exit" variants={containerVariants}>
            ...
            <motion.div variants={itemVariants}>
              ...
              )}
              {
                <div className="flex items-center space-x-2">
                  <Select onValueChange={(value) => handleInputChange("topic", value)} name="topic" value={formData.topic}>
                    <SelectTrigger className="w-full" disabled={customSubtopic !== ""}>
      ...
      case 2:
        return (
          <motion.div key={"step2"} initial="hidden" animate="visible" exit="exit" variants={containerVariants}>
            <motion.div className="flex items-center mb-4" variants={itemVariants}>
              ...
              {(isSubscribed && customTopic === "") || !isSubscribed ? (
                <div className="flex items-center space-x-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      {subtopics[formData.topic as keyof typeof subtopics]?.map((subtopic) => (
                  ...
                  {isSubscribed && formData.subtopic && (
                    <Button onClick={clearSubtopic} variant="outline" size="icon">
                      <X className="h-4 w-4" />
                    </Button>
                  )}
                </div>
      ...
      case 3:
        return (
          <motion.div key={"step3"} initial="hidden" animate="visible" exit="exit" variants={containerVariants}>
            <motion.div className="flex items-center mb-4" variants={itemVariants}>
              <Clock className="w-6 h-6 mr-2 text-green-500" />
              <h2 className="text-2xl font-bold">Select Duration</h2>
            </motion.div>
            <motion.div>
              <Select name="duration" onValueChange={(value) => handleInputChange("duration", value)} value={formData.duration}>
      ...
      case 4:
        return (
          <motion.div key={"step4"} initial="hidden" animate="visible" exit="exit" variants={containerVariants}>
            <motion.div className="flex items-center mb-4" variants={itemVariants}>
      ...
      case 5:
        return (
          <motion.div key={"step5"} initial="hidden" animate="visible" exit={"exit"} variants={containerVariants}>
            <motion.div className="flex items-center mb-4" variants={itemVariants}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <CardContent>
          <div className="mb-6 flex justify-center">
            {[1, 2, 3, 4, 5].map((i) => (
              <div key={i} className={`w-3 h-3 rounded-full mx-1 ${i <= step ? "bg-indigo-500" : "bg-gray-300"}`} />
            ))}
          </div>
          <AnimatePresence mode="wait">{renderStep()}</AnimatePresence>
          <motion.div className="mt-6 flex justify-between" initial="hidden" animate="visible">
            {step > 1 && (
              <motion.div>
